    ...
    if (swagger.paths) {
      ...
      if (options && typeof options != 'object') {
        ...
        Object.keys(swagger.paths).forEach(function (url) {
          ...
          var customRegex = url.replace(/{(\w+)}/gi, function myFunction(
            wholeString,
            variableName
          ) {
...
var getErrors = function (
      ...
      case 'query':
        if (
          ...
          if (
            ...
            errorsToReturn.push(
              new Error(
                'Parameter : ' + parameter.name + ' does not respect its type.'
              )
            );
          }
          ...
          if (
            ...
            errorsToReturn.push(
              new Error(
                'Parameter : ' + parameter.name + ' has an unauthorized value.'
              )
            );
          }
        }
  ...
  if (this._strict) {
    ...
    var isThereVariablesThatShouldNotBeSpecified = function (
      bodyParameters,
      whereToSearch
    ) {
      if (whereToSearch['$ref']) {
        whereToSearch = getObjectFromSwaggerReference.call(
          this,
          whereToSearch['$ref']
        );
      }
    ...
    Object.keys(queryParameters).forEach(function (variableName, index) {
      if (!isPresentWithinTheSwagger('query', variableName)) {
        errorsToReturn.push(
          new Error('Parameter : ' + variableName + ' should not be specified.')
        );
      }
    });
...
var complexTypeChecking = function (objectToControl, swaggerModel) {
  ...
  if (swaggerModel['$ref']) {
    swaggerModel = getObjectFromSwaggerReference.call(
      this,
      swaggerModel['$ref']
    );
  }
...
var getGeneriqueMiddleware = function (swaggerParameters) {
  ...
  return function (req, res, next) {
    ...
    var errorsToReturn = getErrors.call(
      thisReference,
      swaggerParameters,
      queryParameters,
      pathParameters,
...
SwaggerInputValidator.prototype.all = function () {
  ...
  return function (req, res, next) {
    ...
    var swaggerParameters = getRequiredParameters.call(
      thisReference,
      verb,
      parsingParameters.swaggerPath
    );

    var queryParameters = req.query;
    var pathParameters = getPathParametersFromUrl.call(
      thisReference,
      url,
      parsingParameters
    );
